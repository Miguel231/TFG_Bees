...
│       └── 04_honey_super.ipynb               Honey super placement prediction (LightGBM)
├── api/                                   # Deployed prediction system
│   ├── main.py                                FastAPI backend (/predict, /weekly-run, /weekly-alza-update)
│   ├── dashboard.py                           Streamlit dashboard (6 tabs: swarm, alzas, sensors, trends, models, history)
│   ├── inference.py                           LSTM inference pipeline (replicates notebook feature engineering)
...
Place (or symlink) the unified CSV in `data/`:
...
The actual CSVs live at repo-root level under `../csv/`. The notebooks and the alza update script (`update_alza_features.py`) resolve the path automatically.

The notebook `01_swarm_multihorizon.ipynb` additionally needs a preprocessed daily features file:
...
The honey super notebook (`04_honey_super.ipynb`) and the API use:

```
# repo root
daily_features_final.csv      # Daily feature matrix for all 17 alza hives (auto-updated weekly)
```
...
The `api/` folder contains a production-ready prediction system that runs locally and is updated automatically each week.
